...
assert.deepEqual(weekly.points[0].expenses, [expenses[0]]);
assert.deepEqual(weekly.points[1].expenses, []);
...
assert.deepEqual(monthly.points.at(-1).expenses, expenses.slice(0, 2));
...
assert.deepEqual(weeks.points[4].expenses.map(expense => expense.date), ["2026-08-31", "2026-09-06"]);
assert.deepEqual(weeks.points[5].expenses.map(expense => expense.date), ["2026-09-07", "2026-09-08"]);
for (const series of [weekly, monthly, weeks]) {
  for (const point of series.points) {
    assert.equal(point.expenses.reduce((sum, expense) => sum + Math.round(expense.amount / expense.people), 0), point.amount);
  }
}
